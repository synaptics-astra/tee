		...
		TEEC_Property*		property)
{
	...
	if (property) {
		res = TEEC_GetPropertyUint32(property, "CMD_TYPE", &cmdType);
		if (res) {
			error("get TA property failed\n");
			return res;
		}
		res = TEEC_GetPropertyTAType(property, &codeType);
		if (res && res != TEEC_ERROR_ITEM_NOT_FOUND) {
			error("get TA property failed\n");
			return res;
		}
	}
	...
	operation.params[1].value.b = cmdType;

	if(cmdType == TAMgrCmd_FLAG_GETINFO)
		tmpMemoryProperty = TEEC_MEMREF_PARTIAL_INOUT;
	else
		tmpMemoryProperty = TEEC_MEMREF_PARTIAL_INPUT;
		...
		int		size)
{
	...
	n = 0;
	while (n < size) {
		ret = TEMP_FAILURE_RETRY(read(fd, (void*)((unsigned long)buf + n), size - n));
		if (ret < 0) {
		...
		size_t			len)
{
	...
	dstPhyAddr = (void*)((unsigned long)dst->phyAddr + dstOffset);
	srcPhyAddr = (void*)((unsigned long)src->phyAddr + srcOffset);
