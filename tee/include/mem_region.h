...
struct mem_zone {
	...
	uint32_t		attr;	/* perm	*/
	uint32_t		start;
	uint32_t		master_attr[MAX_MASTER_NUM];
};
...
// master_attr
#define MZ_M_MST_PERM			(0x000000ff)
#define MZ_M_MST_ID				(0xff000000)
#define MZ_S_MST_PERM			0
#define MZ_S_MST_ID				24
#define MZ_MASTER_ATTR(perm, master_id) (((master_id) << MZ_S_MST_ID) | \
										((perm) & MZ_M_MST_PERM))
#define MZ_MASTER_ID(master_attr) (((master_attr) & MZ_M_MST_ID) >> MZ_S_MST_ID)
#define MZ_MASTER_PERM(master_attr) (((master_attr) & MZ_M_MST_PERM) >> MZ_S_MST_PERM)
