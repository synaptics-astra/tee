...
void TestCoreCancel(struct MODULE_Cfg* test_cfg)
{
	...
	int 			cmdId = TEST_CMD_CORE_CANCEL;
	int 			case_index = 0;
	...
	pthread_create(&cancel_thread, NULL, thread_for_cancellation,
			(void*)(&operation));
...
void TestCoreCancelAPI(struct MODULE_Cfg *test_cfg)
{
	...
	printf("*******enter core cancel api test*********\n");
	...
	while(1) {
		...
		for(i = 0; i < test_times;i++) {
			TestCoreCancel(&test_cfg[test_index]);
		}
